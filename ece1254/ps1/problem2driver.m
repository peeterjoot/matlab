function problem2driver( doProblemB, figdesc )
   ...
   if ( doProblemB )
      ...
      exp(a)
      f = figure ;
      plot( logN, logFitCheck, logN, logTotalTimes ) ;

      savePlot( f, sprintf( '%s%s.%s', figdesc, 'LogFitCheckAndLogTotal', fileExtension ) ) ;

      f = figure ;
      plot( gridSizes, fitCheck, gridSizes, totalTimes ) ;

      savePlot( f, sprintf( '%s%s.%s', figdesc, 'FitCheckAndTotalTimes', fileExtension ) ) ;
